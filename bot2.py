...
bot = telebot.TeleBot("6921435210:AAFNEomzzA8xaWVunTLRG_L6_1bBQtODHVY")
#geolocator = Nominatim(user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36")
...
def language(message):
    user_id = message.from_user.id
    bot.send_message(user_id, "Выберите язык:", reply_markup=bt.language_kb())
    bot.register_next_step_handler(message, set_language)
def set_language(message):
    user_id = message.from_user.id
    language = message.text
    db.set_user_language(user_id, language)
    bot.send_message(user_id, "Язык успешно установлен.")
